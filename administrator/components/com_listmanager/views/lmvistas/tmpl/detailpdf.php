...
?>
<?php echo $this->__getHelp('DETAIL');?>
...
window.addEvent('load',function(){
	$$('.columns').addEvent('click',function(e){
		if(confirm("<?php echo JText::_('LM_CONFIRM_LAYOUT_CHANGE');?>")){				
			var elem=e.target;
			generateTable($(elem).getProperty('idlo'));
		}
	});
});
function setContentEditor(value){
  ...
  <?php 
  $value='value';
  echo str_replace("'","",$editor->setContent('detail',$$value));
  ?>
}
	...
	<div class="form-horizontal">	
		...
		<div id="editcell" class="tab-content">
			<div id="sec_A" class="tab-pane fade in form-horizontal active">
				...
				<?php
						$editor =JFactory::getEditor();
						echo $editor->display('detailpdf', isset($this->item['detailpdf'])?$this->item['detailpdf']:'', '600','400','100','6');
				?> 
				</div>	
			</div>			
		</div>
...
<input type="hidden" name="idview" value="<?php echo $this->item['id']; ?>" />
...
<input type="hidden" name="task" value="storeDetailpdf" />
