...
class BookProViewBusTrips extends JViewLegacy {
	function display($tpl = null) {
		...
		$this->bus_id= JFactory::getApplication()->getUserStateFromRequest ( 'filter.bus_id', 'filter_bus_id',null);
		...
		if($this->bustrip_id){
			...
			$state->set ( 'filter.bus_id', $this->bus_id );
			
		}
		
		
		
		//if (JFactory::getDate ()->format ( 'Y-m-d' ) == trim($this->start) ) {
