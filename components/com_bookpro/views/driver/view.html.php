...
class BookproViewDriver extends JViewLegacy
{
	...
	function display($tpl = null)
	{
		...
		if(!$this->account->isDriver){
			...
			JFactory::getApplication()->redirect('index.php?option=com_bookpro&view=login&return='.base64_encode('index.php?option=com_bookpro&view=driver'));
		...
		$this->Itemid=JFactory::getApplication()->getUserStateFromRequest('driver.Itemid', 'Itemid');
	...
	function getRoute(){
		...
		$query->select ( 'bt.*' );
		$query->from ( '#__bookpro_bustrip AS bt' );
		$query->innerJoin('#__bookpro_job AS j ON bt.id = j.route_id');
		$query->where ( 'j.cid=' . $this->account->id );
		$query->where('bt.state=1');
		$query->where ( 'DATE_FORMAT(j.date,"%Y-%m-%d")=' . $db->quote ( $this->date ));
		$sql = ( string ) $query;
		$db->setQuery ( $sql );
		$item = $db->loadObject();
		return $item;
