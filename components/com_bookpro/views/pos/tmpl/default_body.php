...
<div>
	...
	<form action="<?php echo JRoute::_("index.php?option=com_bookpro&controller=pos&task=confirm")?>" method="POST" name="posform" id="posform" onsubmit="return submitForm()">	
	<?php if (JFactory::getApplication()->input->get('check')==1) {?>
...
jQuery(document).ready(function($) {
	...
	$(".typeahead").bind("keyup paste", function() {
	    $("#firstname_id").val($(this).val());
	});

	$(".phone").bind("keyup paste", function() {
	    $("#phone_id").val($(this).val());
	});
	
jQuery.fn.bootstrapTypeahead = jQuery.fn.typeahead.noConflict();

var customers = new Bloodhound({
  datumTokenizer: Bloodhound.tokenizers.obj.whitespace('text','email','telephone'),
  queryTokenizer: Bloodhound.tokenizers.whitespace,
  prefetch: 'index.php?option=com_bookpro&controller=customer&task=typeahead&t='+<?php echo strval(time())?>
});
...
jQuery('input.typeahead').typeahead(null, {
  ...
  source: customers
    ...
    console.log(datum.email);
    $('input[name="jform[email]"]').val(datum.email);
    $('input[name="jform[telephone]"]').val(datum.telephone);
    $('input[name="jform[passport]"]').val(datum.passport);
    $('input[name="jform[id]"]').val(datum.id);
    $('#firstname_id').val(datum.text);
    $('#passport_id').val(datum.passport);
    // find the first passenger
			...
			<table class="table" id="pax_table_id">
				<thead>
				  ...
				  <?php if ($this->config->get('ps_passport')){?>
					...
					<?php } ?>
				  <?php if ($this->config->get('ps_phone')){?>
					<td><?php echo JText::_('COM_BOOKPRO_PASSENGER_PHONE')?></td>
					<?php } ?>
				  ...
				  <?php if ($this->config->get('ps_birthday')){?>	
					<td><input type="text" class="input-small birthday"
							name="<?php echo $fieldname?>[birthday]" id="<?php echo 'birthday'?>" /></td>
					<?php } ?>
				  ...
				  <?php if ($this->config->get('ps_passport')){?>
					...
					<?php if ($this->config->get('ps_phone')){?>
					<td><input type="text" name="<?php echo $fieldname?>[phone]" required
							class="input-medium jbpassenger" <?php if ($i==0) echo 'id="phone_id"' ?> /></td>
					<?php } ?>
					
					<?php if ($this->config->get('ps_notes')){?>
				  <td><input type="text" name="<?php echo $fieldname?>[notes]"  class="input-medium jbpassenger" /></td>
					<?php } ?>
